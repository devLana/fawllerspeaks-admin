...
const createDeletedTestPosts = async (
  db: Pool,
  postTags: PostTag[],
  author: PostAuthor
) => {
  ...
  try {
    ...
    console.error("Create Deleted Test Posts Error - ", err);
    throw new Error("Unable to create deleted test posts");
  }
};

export default createDeletedTestPosts;
